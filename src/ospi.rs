//! Octo Serial Peripheral Interface (SPI) bus: A specialized interface used for
//! high-speed communications with external flash memory.

use crate::{
    clocks::Clocks,
    pac::{OCTOSPI1 as OCTOSPI, RCC},
};
...
pub enum ProtocolMode {
    ...
    Quad = 0b011,
    /// All octo IO lines are used for transmit/receive.
    Octo = 0b100,
}
...
impl Default for QspiConfig {
    fn default() -> Self {
        Self {
            ...
            protocol_mode: ProtocolMode::Octo,
...
impl Qspi {
    pub fn new(regs: OCTOSPI, cfg: QspiConfig, clocks: &Clocks) -> Self {
        assert!(
            cfg.dummy_cycles < 32,
        ...
        while regs.osr.read().busy().bit_is_set() {}

        regs.occr.modify(|_, w| unsafe {
            ...
            w.admode().bits(cfg.address_size as u8)
        });
        regs.otcr
        ...
        regs.dcr1.modify(|_, w| unsafe { w.devsize().bits(fsize) }); // todo
        ...
        regs.otcr
            .write(|w| unsafe { w.sshift().bit(sampling_edge as u8 != 0) });
    ...
    pub fn is_busy(&self) -> bool {
        self.regs.osr.read().busy().bit_is_set()
    }
    ...
    pub fn write_indirect(&mut self, addr: u32, data: &[u8]) {
        ...
        unsafe {
            for word in data {
                ptr::write_volatile(&self.regs.odr as *const _ as *mut u8, *word);
            }
        }

        // Wait for the transaction to complete
        while self.regs.osr.read().tcf().bit_is_clear() {}
    ...
    pub fn read_indirect(&mut self, addr: u32, buf: &mut [u8]) -> Result<(), QspiError> {
        ...
        if (self.regs.osr.read().flevel().bits() as usize) < buf.len() {
            return Err(QspiError::Underflow);
        }

        unsafe {
            for word in buf {
                *word = ptr::read_volatile(&self.regs.odr as *const _ as *const u8);
            }
        }
